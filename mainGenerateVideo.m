...
clear
%Gunzip files
files = gunzip('MTP_2023_0004_3T1_seg.nii.gz');
%files_seg = gunzip('BraTS_version\BraTS2021_01516_seg.nii.gz','BraTS_version\');
segvol1 = niftiread("MTP_2023_0004_3T1_seg.nii");
%vol1 = niftiread('Unziped_Dataset\MTP_2023_0004\T1.nii');
segvol2 = niftiread('T2_half_seg.nii');
segtumor_flip = flip(niftiread('BraTS2021_01516_seg.nii'));
seg_tumor = niftiread('BraTS2021_01516_seg.nii');
sug_tumor_twice = seg_tumor + segtumor_flip;
%%
%Find coordinates for segmented tumor
nonzero_indices = find(seg_tumor); % find indices of non-zero elements

% convert linear indices to subscripts
[Sub1, Sub2, Sub3] = ind2sub(size(segtumor_flip),nonzero_indices);
sub1size = size(Sub1);
sub2size = size(Sub2);
sub3size = size(Sub3);

xmid = Sub1(round(sub1size(1)/2));
ymid = Sub2(round(sub2size(1)/2));
zmid = Sub3(round(sub3size(1)/2));
...
noIter = 300;    % Number of iterations in t-variable
...
% fig1 = figure(1);
% fig1.Position = [-1599, -199, 500, 500];
% 
...
% fig2 = figure(2);
% fig2.Position = [-1000, -100, 500, 500];
% 
...
(1-u.^(1/beta)).^gamma);
fprintf('Sum u: %f\n', sum(u(:) .* (u(:)>0.16) )./(sum(u(:)>0.16)));
avg_density(currIter) = sum(u(:) .* (u(:)>0.16) )./(sum(u(:)>0.16));
%  fprintf('Sum u08: %f\n', sum(u(:)>0.08));
%  fprintf('Sum u16: %f\n', sum(u(:)>0.16));
%  fprintf('Sum u24: %f\n', sum(u(:)>0.24));
figure(fig3);
imagesc(u(:,:,102),[cmin, cmax]);
clim([cmin, cmax])
colormap jet; 
colorbar; 
axis equal; 
